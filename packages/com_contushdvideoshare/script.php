...
`catid` varchar(100) CHARACTER SET utf8 NOT NULL
		...
		foreach ($modules as $module) {
			...
			$status->modules[] = array('name' => $name, 'client' => $client, 'result' => $result);
                        $published = 1;
			if($name=="mod_videoplusbanner") {
			  $positionRight="videoplus-banner";
			}
				
			if($name=="mod_videopluscategories") {
			  $positionRight="videoplus-category";
			}
				
			if($name=="mod_videoplusheader") {
			  $positionRight="videoplus-header";
			}
			
			if($name=="mod_hdvideosharecategories") {
			  $positionRight=$position;
			}
			
			if($name=="mod_hdvideosharemodules") {
			  $positionRight=$position;
			}
			
			if($name=="mod_hdvideosharerss") {
			  $positionRight='';
                          $published = 0;
			}
			
			if($name=="mod_videoshare") {
			  $positionRight='';
                          $published = 0;
			}
			
			/** Update module positions into db */
			$query->clear() ->update($db->quoteName('#__modules')) ->set($db->quoteName('published') . ' = ' . $db->quote($published))
			->set($db->quoteName('position') . ' = ' . $db->quote($positionRight)) ->where($db->quoteName('module') . ' = ' . $db->quote($name));
			$db->setQuery($query);
			$db->query();
				
			$query->clear() ->select('id') ->from($db->quoteName('#__modules')) ->where($db->quoteName('module') . ' = ' . $db->quote($name));
			$db->setQuery($query);
			$db->query();
			$mid = $db->loadResult();				
			$db->setQuery("INSERT IGNORE INTO #__modules_menu (moduleid) VALUES ('$mid')");
			$db->query();
		}
			...
			<tbody>
				...
				<tr class="row0">
					...
					<td style="text-align: center;">
						<?php
						// Check installed modules
						$query->clear() ->select('extension_id') ->from($db->quoteName('#__extensions'))

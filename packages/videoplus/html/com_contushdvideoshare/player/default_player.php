...
$video_title = $video_desc = $video_thumb = $memberidvalue = $scrolClass = '';
...
<div id="video-container" class="" itemscope itemid="" itemtype="http://schema.org/VideoObject">
	...
	<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
		<meta itemprop="ratingValue" content="<?php if($this->commentview [0]->ratecount  != 0 ) { echo round ( $this->commentview [0]->rate / $this->commentview [0]->ratecount ); } ?>" />
	...
	<a
		...
		if (version_compare ( JVERSION, '1.6.0', 'ge' )) {
			?>
				<a
		href="<?php
			echo JRoute::_ ( 'index.php?option=com_users&task=user.logout&' . JSession::getFormToken () . '=1&return=' . base64_encode ( JUri::root () ) );
			?>">
	...
	<?php  }
		...
		<?php  }
			...
			if ($player_icons ['enabledownload'] == 1 && $this->htmlVideoDetails->download == 1 && $this->htmlVideoDetails->filepath != "Youtube" && $this->htmlVideoDetails->filepath != "Embed" && $this->htmlVideoDetails->streameroption != "rtmp") {
				?>
	<a class="utility-link embed" href="<?php echo $video_url; ?>"
				target="_blank">
		...
		<script type="text/javascript">
					...
					?>&cmdid=2&video_pageid=" + pageno;
							...
							// Set te random number to add to URL request
									...
									var url = "<?php
					echo JURI::base ();
					?>index.php?option=com_contushdvideoshare&view=player&id="

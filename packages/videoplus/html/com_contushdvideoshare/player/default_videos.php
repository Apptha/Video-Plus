...
<div class="section clearfix ">
	...
	for($coun_tmovie_post = 1; $coun_tmovie_post <= 3; $coun_tmovie_post ++) {
		...
		<h2 class="home-link hoverable">
			<a
				href="<?php
			echo JRoute::_ ( "index.php?Itemid=" . $Itemid . "&amp;option=com_contushdvideoshare&amp;view=featuredvideos" );
			?>"
				title="
<?php
			echo JText::_ ( 'HDVS_FEATURED_VIDEOS' );
			?>"> <?php
			echo JText::_ ( 'HDVS_FEATURED_VIDEOS' );
			?>
						</a>
			...
			for($i = 0; $i < $totalrecords; $i ++) {
				...
				if ($this->rs_playlist1 [0] [$i]->filepath == "File" || $this->rs_playlist1 [0] [$i]->filepath == "FFmpeg" || $this->rs_playlist1 [0] [$i]->filepath == "Embed") {
					if (isset ( $this->rs_playlist1 [0] [$i]->amazons3 ) && $this->rs_playlist1 [0] [$i]->amazons3 == 1) {
						$src_path = $dispenable ['amazons3link'] . $this->rs_playlist1 [0] [$i]->thumburl;
					} else {
				...
				?>
							<li class="video-item featured_gutterwidth"><div class="video_thumb_wrap">
							<a
				class="info_hover featured_vidimg" rel="htmltooltip"
				href="<?php
				...
				?>">
					...
					src="<?php echo $src_path; ?>" width="145" height="80" title=""
					alt="thumb_image" />
					<?php 
				...
				if ($dispenable ['viewedconrtol'] == 1) {
					...
					echo $this->rs_playlist1 [0] [$i]->times_viewed;
					?> <?php
...
<?php  echo JHTML::_ ( 'string.truncate', (strip_tags ( $this->rs_playlist1 [0] [$i]->description )), 120 ); ?>
							</p>
						<?php }
		...
		<h2 class="home-link hoverable">
			<a
				href="<?php
			echo JRoute::_ ( "index.php?Itemid=" . $Itemid . "&amp;option=com_contushdvideoshare&amp;view=popularvideos" );
			?>"
				title="<?php
			echo JText::_ ( 'HDVS_POPULAR_VIDEOS' );
			?>"><?php
			echo JText::_ ( 'HDVS_POPULAR_VIDEOS' );
			?>
						</a>
			...
			for($i = 0; $i < $totalrecords; $i ++) {
				...
				if ($this->rs_playlist1 [2] [$i]->filepath == "File" || $this->rs_playlist1 [2] [$i]->filepath == "FFmpeg" || $this->rs_playlist1 [2] [$i]->filepath == "Embed") {
					if (isset ( $this->rs_playlist1 [2] [$i]->amazons3 ) && $this->rs_playlist1 [2] [$i]->amazons3 == 1) {
						$src_path = $dispenable ['amazons3link'] . $this->rs_playlist1 [2] [$i]->thumburl;
					} else {
				...
				?>
							<li class="video-item popular_gutterwidth"> <div class="video_thumb_wrap"><a
				class=" info_hover featured_vidimg" rel="htmltooltip1"
				href="<?php
				...
				if ($dispenable ['viewedconrtol'] == 1) {
					?>
										<span class="floatright viewcolor">
											<?php echo $this->rs_playlist1[2][$i]->times_viewed; ?> <?php
					echo JText::_ ( 'HDVS_VIEWS' );
					?></span>
									<?php
				}
...
<?php
	...
	<div id="video-grid-container_rec" class="clearfix">
		<h2 class="home-link hoverable">
			...
			?>"
				title="<?php echo JText::_('HDVS_RECENT_VIDEOS'); ?>"> <?php
			echo JText::_ ( 'HDVS_RECENT_VIDEOS' );
			?></a>
		</h2>
		<ul class="ulvideo_thumb clearfix">
						<?php
			...
			for($i = 0; $i < $totalrecords; $i ++) {
				...
				if ($this->rs_playlist1 [1] [$i]->filepath == "File" || $this->rs_playlist1 [1] [$i]->filepath == "FFmpeg" || $this->rs_playlist1 [1] [$i]->filepath == "Embed") {
					if (isset ( $this->rs_playlist1 [1] [$i]->amazons3 ) && $this->rs_playlist1 [1] [$i]->amazons3 == 1) {
						$src_path = $dispenable ['amazons3link'] . $this->rs_playlist1 [1] [$i]->thumburl;
					} else {
				...
				?>
							<li class="video-item recent_gutterwidth"><div class="video_thumb_wrap"><a
				class=" info_hover featured_vidimg" rel="htmltooltip2"
				href="<?php
...
<?php
				...
				if ($dispenable ['viewedconrtol'] == 1) {
					?>
										<span class="floatright viewcolor"><?php
					echo $this->rs_playlist1 [1] [$i]->times_viewed;
					?> <?php
					echo JText::_ ( 'HDVS_VIEWS' );
					?></span>
									<?php
